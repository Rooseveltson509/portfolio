    ...
    Radar.prototype.draw = function () {
      ...
      for (var i = min; i <= max; i += this.settings.step) {
        ...
        if (this.settings.showAxisLabels)
          this.cxt.fillText(
            i,
            this.width / 2 + this.settings.step * spacing * i + 4,
            this.height / 2 - 2
          );
      }

      var size = 0;
      for (var key in this.settings.values) size += 1;

      for (var i = 0; i < size; i += 1) {
        this.cxt.beginPath();
        this.cxt.moveTo(this.width / 2, this.height / 2);
        var x =
          this.width / 2 + Math.cos(Math.PI * 2 * (i / size)) * spacing * max;
        var y =
          this.height / 2 + Math.sin(Math.PI * 2 * (i / size)) * spacing * max;
        this.cxt.lineTo(x, y);
        this.cxt.stroke();
      }
      ...
      $.each(this.settings.values, function (key, val) {
        var x =
          that.width / 2 + Math.cos(Math.PI * 2 * (i / size)) * spacing * val;
        var y =
          that.height / 2 + Math.sin(Math.PI * 2 * (i / size)) * spacing * val;
        if (first) {
          that.cxt.moveTo(x, y);
          end.x = x;
          end.y = y;
      ...
      $.each(this.settings.values, function (key, val) {
        ...
        var textY =
          that.height / 2 +
          Math.sin(Math.PI * 2 * (i / size)) * spacing * val * 1.5;
